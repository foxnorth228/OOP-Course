...
namespace StarMap
{
    public class StarmapMeshRenderer : MonoBehaviour
    {
        ...
        public AnimationCurve sizeByMagnitudeCurve = new(
            new Keyframe(0, 4f),
            new Keyframe(7, 1f));
        ...
        void Start()
        {
            ...
            List<Star> starsList = cullBelowHorizon ?
                Starmap.GetStarsCulledBelowHorizon(stars, transform.InverseTransformDirection(Vector3.up), cullBelowHorizonOffset) :
                new List<Star>(stars);
            ...
            if (useCubicSplitting)
            {
                ...
                {
                    List<Star[]> splits = Starmap.SplitToMax(cs, Starmap.MESH_STAR_COUNT_LIMIT);
                    AddStarsToScene(splits);
                }
            }
            else
            {
                List<Star[]> splits = Starmap.SplitToMax(starsList, Starmap.MESH_STAR_COUNT_LIMIT);
                AddStarsToScene(splits);
            }
        }

        void AddStarsToScene(List<Star[]> splits)
        {
            foreach (var chunk in splits)
            {
                Mesh m = GenerateStarsAsStaticQuadsMesh(chunk);
                GameObject go = new("Stars");
